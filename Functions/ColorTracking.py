    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up: # 当首次检测到物体时               
                ...
                setBuzzer(0.1)               
                result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0, 1000) # 不填运行时间参数，自适应运行时间
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                ...
                if start_pick_up: #如果物体没有移动一段时间，开始夹取
                    ...
                    if not __isRunning: # 停止以及退出标志位检测
                        continue
                    Board.setBusServoPulse(1, servo1 - 280, 50)  # 爪子张开
                    # 计算夹持器需要旋转的角度
                    servo2_angle = getAngle(world_X, world_Y, rotation_angle)
                    Board.setBusServoPulse(2, servo2_angle, 50)
                    time.sleep(0.2)
                    
                    if not __isRunning:
                        continue
                    AK.setPitchRangeMoving((world_X, world_Y, 2), -90, -90, 0, 200)  # 降低高度
                    time.sleep(0.4)
                    
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1, 100)  # 夹持器闭合
                    time.sleep(0.2)
                    
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(2, 500, 50)
                    AK.setPitchRangeMoving((world_X, world_Y, 12), -90, -90, 0, 200)  # 机械臂抬起
                    time.sleep(0.2)
                    
                    if not __isRunning:
                        continue
                    # 对不同颜色方块进行分类放置
                    result = AK.setPitchRangeMoving((coordinate[detect_color][0], coordinate[detect_color][1], 12), -90, -90, 0, 500)   
